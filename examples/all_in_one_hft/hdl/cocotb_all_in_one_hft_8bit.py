"""Cocotb tests for all_in_one_hft_parser_8bit."""
import struct
...
@cocotb.test()
async def throughput_one_byte_per_cycle(dut):
    """On an 8-bit bus every input byte is processed in exactly one cycle."""
    cocotb.start_soon(Clock(dut.clk, CLOCK_PERIOD_NS, unit="ns").start())
    source = AxiStreamSource(AxiStreamBus.from_prefix(dut, "s_axis"), dut.clk, dut.rst)
    await reset_dut(dut)

    msg = make_system_event()
    frame = make_frame([msg])  # 62 + 2 + 12 = 76 bytes
    ...
    assert handshake_count == len(frame), (
        f"Expected {len(frame)} handshakes (one per byte), got {handshake_count}"
    )
