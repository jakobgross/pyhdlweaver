// eth_ip_forward_udp_512bit generated with pyhdlweaver by Jakob Gross
// https://github.com/jakobgross/pyhdlweaver

module eth_ip_forward_udp_512bit #(
...
logic [15:0] eth_ethertype_comb;
logic [7:0] ip_version_ihl_comb;
logic [15:0] ip_total_length_comb;
logic [15:0] ip_flags_frag_comb;
logic [7:0] ip_protocol_comb;
logic [31:0] ip_src_comb;
...
always_comb begin
  ...
  if (parse_fire && beat_count == PARSE_FINAL_BEAT) begin
    ...
    ip_src_comb[7:0] = s_axis_tdata[239:232];
  end
end
...
always_comb begin
  ...
  if (parse_fire && beat_count == PARSE_FINAL_BEAT) begin
    ...
    ip_dst_comb[7:0] = s_axis_tdata[271:264];
  end
end
...
always_comb begin
  // Combine drop checks.
  drop_next = 1'b0;
  drop_next = drop_next | (ip_protocol_comb != 8'h11);
end
    ...
    case (state)
      ST_PARSE: begin
        ...
        if (parse_fire) begin
          case (beat_count)
            0: begin
              eth_ethertype_reg[15:8] <= s_axis_tdata[103:96];
              eth_ethertype_reg[7:0] <= s_axis_tdata[111:104];
              ip_version_ihl_reg[7:0] <= s_axis_tdata[119:112];
              ip_total_length_reg[15:8] <= s_axis_tdata[135:128];
              ip_total_length_reg[7:0] <= s_axis_tdata[143:136];
              ip_flags_frag_reg[15:8] <= s_axis_tdata[167:160];
              ip_flags_frag_reg[7:0] <= s_axis_tdata[175:168];
              ip_protocol_reg[7:0] <= s_axis_tdata[191:184];
              ip_src_reg[31:24] <= s_axis_tdata[215:208];
              ip_src_reg[23:16] <= s_axis_tdata[223:216];
              ip_src_reg[15:8] <= s_axis_tdata[231:224];
              ip_src_reg[7:0] <= s_axis_tdata[239:232];
              ip_dst_reg[31:24] <= s_axis_tdata[247:240];
              ip_dst_reg[23:16] <= s_axis_tdata[255:248];
              ip_dst_reg[15:8] <= s_axis_tdata[263:256];
              ip_dst_reg[7:0] <= s_axis_tdata[271:264];
            end
            default: begin
          ...
          if (beat_count == PARSE_FINAL_BEAT) begin
            ...
            if (s_axis_tlast && input_valid_bytes_comb < PAYLOAD_START_BYTE) begin
              ...
              fields_fresh <= 1'b1;
              parser_drop <= 1'b1;
              if ((ip_protocol_comb != 8'h11)) non_udp_drop_count <= non_udp_drop_count + 32'd1;
