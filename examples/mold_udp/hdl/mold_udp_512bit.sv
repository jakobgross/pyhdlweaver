...
assign body_needs_input = (state == ST_MSG_BODY) &&
                          (scratch_count_reg < 8'(KEEP_WIDTH)) &&
...
always_comb begin
  ...
  if (state == ST_MSG_BODY && scratch_count_reg != '0) begin
    ...
    else if (scratch_count_reg < 8'(KEEP_WIDTH))
        ...
        if (parse_fire) begin
          ...
          if (beat_count == PARSE_FINAL_BEAT) begin
            ...
            if (input_valid_bytes_comb < OUTER_TAIL_START) begin
              ...
              scratch_count_reg <= 8'(outer_tail_count_comb);
      ...
      ST_MSG_HDR: begin
        if (scratch_count_reg == '0 && s_axis_tvalid && s_axis_tlast && input_valid_bytes_comb == 0) begin
          ...
          if (consume_from_input_comb) begin
            if (s_axis_tuser)
              sticky_tuser_reg <= 1'b1;
            scratch_data_reg <= 1024'(s_axis_tdata) >> 8;
            scratch_count_reg <= 8'(input_valid_bytes_comb - 1);
            scratch_last_reg <= s_axis_tlast;
          end else begin
        ...
        if (scratch_count_reg == '0 || body_needs_input) begin
          ...
          if (scratch_load_fire) begin
            ...
            if (scratch_count_reg == '0) begin
              scratch_data_reg <= 1024'(s_axis_tdata);
            end else begin
              for (int i = 0; i < KEEP_WIDTH; i++) begin
            ...
            scratch_count_reg <= scratch_count_reg + 8'(input_valid_bytes_comb);
            scratch_last_reg <= s_axis_tlast;
          ...
          scratch_count_reg <= scratch_count_reg - 8'(body_out_bytes_comb);
          ...
          if (body_truncated_comb) begin
            ...
            if (msg_remaining_reg <= 1) begin
              msg_remaining_reg <= '0;
              if (scratch_count_reg > 8'(body_out_bytes_comb)) begin
              ...
              if (scratch_count_reg == 8'(body_out_bytes_comb) && scratch_last_reg) begin
